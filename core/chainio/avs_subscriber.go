...
type AvsSubscriberer interface {
	SubscribeToNewTasks(newTaskCreatedChan chan *cstaskmanager.ContractKYTTaskManagerNewTaskCreated) event.Subscription
	SubscribeToTaskResponses(taskResponseLogs chan *cstaskmanager.ContractKYTTaskManagerTaskResponded) event.Subscription
	ParseTaskResponded(rawLog types.Log) (*cstaskmanager.ContractKYTTaskManagerTaskResponded, error)
}
...
func BuildAvsSubscriberFromConfig(config *config.Config) (*AvsSubscriber, error) {
	return BuildAvsSubscriber(
		config.KYTRegistryCoordinatorAddr,
...
func (s *AvsSubscriber) SubscribeToNewTasks(newTaskCreatedChan chan *cstaskmanager.ContractKYTTaskManagerNewTaskCreated) event.Subscription {
...
func (s *AvsSubscriber) SubscribeToTaskResponses(taskResponseChan chan *cstaskmanager.ContractKYTTaskManagerTaskResponded) event.Subscription {
...
func (s *AvsSubscriber) ParseTaskResponded(rawLog types.Log) (*cstaskmanager.ContractKYTTaskManagerTaskResponded, error) {
	return s.AvsContractBindings.TaskManager.ContractKYTTaskManagerFilterer.ParseTaskResponded(rawLog)
}
